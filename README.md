...
### Editable prompts and the cache

If prompts are edited at runtime — a prompt library, an admin UI, anything that
writes to your `PromptStore` — turn this on:

```ts
const gw = new Gateway({ ...cfg, invalidateCacheOnPromptChange: true });
```

Without it, **publishing a prompt changes nothing for any input already cached**
until the TTL expires (24h by default). The cache is read *before* the prompt is
loaded, and the prompt is not part of the key, so the admin sees a successful
edit while production keeps serving output generated by the previous version.
Nothing reports a problem. That is the worst failure shape an admin tool can
have, and it is why this exists.

With it on, the key carries a fingerprint of the resolved prompt — body,
`modelHint`, `providerOverride` and `temperature` — so an edit makes prior
entries unreachable rather than stale. Repointing a prompt at a different model
invalidates too, since that changes the answer just as surely as editing the
text.

**The cost, which is why it is opt-in rather than the default:** the prompt has
to be loaded before the cache can be consulted, so every cache *hit* gains a
`getPrompt` round-trip. Deployments running static `promptDefaults` with no
prompt store gain nothing here and should leave it off.

Two notes:

- **Keys are unchanged when off**, so upgrading does not cold-start an existing
  cache. Turning it *on* invalidates every existing entry once, by design.
- It fingerprints rather than versioning. A version number is only correct while
  every writer remembers to bump it, and one that forgets reintroduces exactly
  this bug — silently. A hash cannot go stale, and needs nothing of your store.
